import { BsWhatsapp, BsFillArrowUpRightCircleFill } from "react-icons/bs";
import { IoMdSchool } from "react-icons/io";
import { useEffect, useRef, useState } from "react";
import { AiOutlineDown } from "react-icons/ai";
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "../ui/accordion";
import { FadeIn } from "../ui/FadeIn";
import { generateSliceIndexes, formatPaketPrice } from "../../utils/Static";
import Footer from "../Footer";
import CarouselProgramBelajar from "../CarouselProgramBelajar";
import programBelajarData from "../../database/program_belajar.json";
...
const { program_belajar } = programBelajarData;
const { sistem_belajar: sistemBelajar } = sistemBelajarData;
const { faq } = faqData;
const { program_yessles } = programData;

const ProgBelPageContent = ({ slug }) => {
  const [activeLink, setActiveLink] = useState(null);
  const [startIndex, endIndex] = generateSliceIndexes();
  const mata_pelajaran = useRef(null);
  const sistem_belajar_ref = useRef(null);
  const paket_belajar = useRef(null);
  const kenapa_yessles = useRef(null);

  const data = program_belajar.find((p) => p.slug === slug);

  const sidebarLinks = [
    { label: "Mata Pelajaran", ref: mata_pelajaran, id: "mata_pelajaran" },
    { label: "Sistem Belajar", ref: sistem_belajar_ref, id: "sistem_belajar" },
    { label: "Paket Belajar", ref: paket_belajar, id: "paket_belajar" },
  ...
  const linkToSemuaMapel = () => {
    window.open(
      "https://drive.google.com/file/d/1Lw2J9AldSgcy2PSBrXzribur3lFnwF6A/view?usp=sharing",
      "_blank",
      "noopener noreferrer"
    );
  };
          ...
          <div className="rounded-xl bg-yellow-50 w-full md:w-[52%] h-[200px] md:h-full overflow-hidden items-center justify-center flex relative">
            ...
            <div className="absolute bottom-0 right-0 p-3 md:p-4 gap-y-2 flex flex-col items-end">
              <div className="flex flex-row flex-wrap gap-2 justify-end">
                {data?.mapel?.opsi.slice(startIndex, endIndex).map((opsi, i) => (
                  <div key={i} className="px-3 py-1 rounded-md bg-yl-10/30 text-white text-[11px] md:text-sm">
                    {opsi?.label}
                  </div>
                ))}
      ...
      <section id="new_form">
        <div className="flex flex-col md:flex-row w-full h-full px-[5%] md:px-[8%] gap-y-4 md:gap-x-4 font-lexend mt-8">
          ...
          <div className="w-full md:w-[240px] md:self-start md:sticky md:top-28 flex-shrink-0">
            ...
            <div className="hidden md:flex flex-col rounded-2xl border border-yl-60/15 bg-white overflow-hidden shadow-[0_10px_40px_rgba(27,28,87,0.08)]">
              ...
              <div className="bg-yl-60 px-5 py-4 flex flex-col gap-y-0.5">
                <span className="text-white/70 text-[11px] font-lexend">Program Belajar</span>
                <h2 className="text-white font-bold font-lexend text-[15px] leading-tight">{data?.type}</h2>
              </div>
          ...
          <div className="flex w-full md:w-[85%] flex-col md:px-7">
            {/* Mata Pelajaran */}
            <FadeIn direction="up">
              <section className="w-full h-fit mb-8 md:mb-12" ref={mata_pelajaran} id="mata_pelajaran">
                <div className="flex flex-col gap-y-1 mb-4">
                  <h1 className="text-[20px] md:text-[24px] font-bold text-yl-20 font-lexend">Bisa Pilih Mata Pelajaran</h1>
                  <p className="text-[13px] md:text-[14px] text-gray-500 font-lexend">
                    Pilih fokus belajar sesuai kebutuhan si buah hati.
                  </p>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-2 xl:grid-cols-3 w-full h-fit gap-3 md:gap-4">
                  {data?.mapel?.opsi.map((opsi, i) => (
                    <div
                      key={i}
                      className="group relative w-full h-[120px] md:h-[150px] bg-white border border-yl-60/15 rounded-2xl p-3 md:p-4 flex flex-col justify-between overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_30px_rgba(82,119,115,0.15)] hover:border-yl-60/40"
                    >
                      <div className="absolute -right-6 -top-6 w-16 h-16 rounded-full bg-yl-60/5 group-hover:bg-yl-60/10 transition-colors" />
                      <div className="relative flex items-center justify-center w-10 h-10 md:w-11 md:h-11 rounded-xl bg-yl-60/10 text-yl-60 group-hover:bg-yl-60 group-hover:text-white transition-colors flex-shrink-0">
                        <IoMdSchool className="size-[20px] md:size-[24px]" />
                      </div>
                      <div className="relative flex flex-col">
                        <h2 className="text-[13px] md:text-[16px] font-semibold text-yl-20 font-lexend leading-tight">
                          {opsi?.label}
                        </h2>
                        {opsi?.sub_label && (
                          <span className="text-[11px] md:text-[12px] text-gray-400 leading-tight mt-0.5">
                            {opsi.sub_label}
                          </span>
                        )}
                      </div>
                  ...
                  <div
                    ...
                    className="group relative w-full h-[120px] md:h-[150px] rounded-2xl p-3 md:p-4 flex flex-col justify-between overflow-hidden cursor-pointer bg-yl-60 text-white transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_30px_rgba(82,119,115,0.3)] hover:bg-yl-10"
                  >
                    <div className="absolute -right-6 -top-6 w-16 h-16 rounded-full bg-white/10 group-hover:bg-white/15 transition-colors" />
                    <div className="relative flex items-center justify-center w-10 h-10 md:w-11 md:h-11 rounded-xl bg-white/15 text-white flex-shrink-0">
                      <BsFillArrowUpRightCircleFill className="size-[20px] md:size-[24px] group-hover:scale-110 transition-transform" />
                    </div>
                    <div className="relative flex flex-col">
                      <h2 className="text-[13px] md:text-[16px] font-semibold font-lexend leading-tight">
                        Lihat Lebih Detail
                      </h2>
                      <span className="text-[11px] md:text-[12px] text-white/70 leading-tight mt-0.5">
                        Semua mata pelajaran
                      </span>
                    </div>
                  </div>
            ...
            {/* Paket Belajar */}
            <FadeIn direction="up">
              <section className="w-full h-fit mb-8 md:mb-12" ref={paket_belajar} id="paket_belajar">
                <h1 className="text-[20px] md:text-[24px] font-bold text-yl-20 font-lexend">Paket Belajar</h1>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 w-full font-lexend mt-4">
                  {program_yessles
                    ?.filter((d) => d.type === "paket_belajar")
                    .map((d, i) => (
                      <a
                        key={i}
                        href={`/paket/${d.slug}`}
...
export default ProgBelPageContent;
